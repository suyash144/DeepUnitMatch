...
session_to_find_matches_between = (
    5,
    6,
)  # RecSes numbers of the two sessions to find matches between

conn = sqlite3.connect(matchtables_db_path)
mt = pd.read_sql_query(
    "SELECT ID1,ID2,RecSes1,RecSes2,NBProb18mice,newISI FROM AL032_19011111882_2", conn
)
...
print(
    f"Found {len(matches)} matches between sessions {session_to_find_matches_between[0]} and {session_to_find_matches_between[1]}"
)
